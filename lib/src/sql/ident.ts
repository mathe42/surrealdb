import { alt, char, ParserFn, takeWhile, tuple, takeUntilEsc } from "./_base.ts"
...
const ident_bracketsParser = tuple(char(BRACKET_L), takeWhile(chr=>chr!=BRACKET_R), char(BRACKET_R))
...
const ident_backtickParser = tuple(char('`'), takeUntilEsc('\\`', '`'), char('`'))
